...
<Note>
  Note that this requires an object so the Mintlify playground won't work with a given metadata as it only accepts strings. For example, an example metadata would be:
  ```
  {
    "name": 'James.html',
