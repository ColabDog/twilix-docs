...
For users - before flicking this on as `true` in production, here are 
...
In order to turn it on, one only needs to set `includeModeration` to `true`.

You can easily turn in the parameters found in 

<CardGroup cols={2}>
    <Card title="Turn on content moderation for Generative QA" icon="link" href="/api-reference/features/generative-q&a">
